...
function DaemonSet() {
  ...
  navigator.clipboard.writeText(Daemon).then(() => {
    alert("Daemon YAML copied to clipboard!");
  });
};

const downloadYAML = () => {
  const blob = new Blob([Daemon], { type: "text/yaml" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "Daemon.yaml";
  a.click();
  ...
  const [dnsPolicy, setDnsPolicy] = useState("ClusterFirst");
  const [runAsUser, setRunAsUser] = useState(1000);
  const [fsGroup, setFsGroup] = useState(2000);
  const [advanceType, setAdvanceType] = useState("RollingUpdate");
...
`;
  return (
    <>
      <h1 className="text-center mt-5">Kubernetes DaemonSet Generator</h1>
      <p className="text-center mb-5">Generate a Kubernetes DaemonSet YAML file with custom metadata and labels.</p>
      <div className="container">
                      ...
                      <div className="accordion-collapse collapse show" id="collapseSpec" aria-labelledby="headingSpec" data-bs-parent="#accordionSpec">
                        <div className="accordion-body">
                          ...
                          <input 
                            ...
                            <select className="form-select mb-3" aria-label="Default select example" onChange={(e) => setHostNetwork(e.target.value === 'true')}>
                              <option value="false">False</option>
                              <option value="true">True</option>
                            </select>
                            ...
                            <select className="form-select mb-3" aria-label="Default select example"
                              onChange={(e) => setDnsPolicy(e.target.value)}>
                            ...
                            <strong >Advance Type</strong> <br />
                            <strong className="ms-3">Type</strong>
                            <select className="form-select mb-3 ms-3" aria-label="Default select example" onChange={(e) => setAdvanceType(e.target.value)}>
        ...
        <div className="result">
          ...
          <div className="cmds">
            <h5>Commands</h5>
            <div className="cmd bg-light mb-4 align-content-center " style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl apply -f daemonset.yaml</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl get daemonset</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl get describe daemonset {name}</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl delete -f daemonset.yml</div>
          </div>
        </div>
      </div>
    </>
