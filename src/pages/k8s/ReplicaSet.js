...
function ReplicaSet() {
  ...
  navigator.clipboard.writeText(replicaSet).then(() => {
    alert("Replicaset YAML copied to clipboard!");
  });
};

const downloadYAML = () => {
  const blob = new Blob([replicaSet], { type: "text/yaml" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "replicaSet.yaml";
  a.click();
  URL.revokeObjectURL(url);
};

  const [api, setAPI] = useState("");
  const [kind, setKind] = useState("ReplicaSet");
...
`;
  return (
    <>
      <h1 className="text-center mt-5">Kubernetes ReplicaSet Generator</h1>
      <p className="text-center mb-5">Generate a Kubernetes ReplicaSet YAML file with custom metadata and labels.</p>
      <div className="container">
                    ...
                    <div className="accordion-item">
                      ...
                      <div className="accordion-collapse collapse show" id="collapseSpec" aria-labelledby="headingSpec" data-bs-parent="#accordionSpec">
                        <div className="accordion-body">
                          ...
                          <input type="number" className="form-control mb-3" placeholder="1" value={replicas} onChange={(e) => setReplicas(e.target.value)} />
        ...
        <div className="result">
          ...
          <div className="cmds">
            <h5>Commands</h5>
            <div className="cmd bg-light mb-4 align-content-center " style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl apply -f replicaset.yaml</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl get <replicaset></replicaset></div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl get describe replicaset {name}</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl delete -f replicaset.yml</div>
          </div>
        </div>
      </div>
    </>
