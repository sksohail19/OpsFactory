...
function StatefulSet() {
  ...
  navigator.clipboard.writeText(stateFulSet).then(() => {
    alert("stateFulSet YAML copied to clipboard!");
  });
};

const downloadYAML = () => {
  const blob = new Blob([stateFulSet], { type: "text/yaml" });
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = "stateFulSet.yaml";
  a.click();
  URL.revokeObjectURL(url);
};

  const [api, setAPI] = useState("");
  const [kind, setKind] = useState("StatefulSet");
  ...
  const [purpose, setPurpose] = useState("statefulset");
  ...
  const [pvcName, setPVCName] = useState("my-pvc");
...
`;
  return (
    <>
      <h1 className="text-center mt-5">Kubernetes Stateful Set Generator</h1>
      <p className="text-center mb-5">Generate a Kubernetes Stateful Set YAML file with custom metadata and labels.</p>
      <div className="container">
                      ...
                      <div className="accordion-collapse collapse show" id="collapseSpec" aria-labelledby="headingSpec" data-bs-parent="#accordionSpec">
                        <div className="accordion-body">
                          ...
                          <select className="form-select mb-3 ms-3" aria-label="Default select example"
                            ...
                            <strong className="ms-3">Max Unavailable</strong>
                            <input type="text" className="form-control mb-3 ms-3" placeholder="50%" value={maxUnavailable} onChange={(e) => setMaxUnavailable(e.target.value)} />
                            <strong className="ms-3">Max Surge</strong>
                            <input type="text" className="form-control mb-3 ms-3" placeholder="50%" value={maxSurge} onChange={(e) => setMaxSurge(e.target.value)} />
                            <strong className="ms-3">Min Ready Seconds</strong>
                            <input type="number" className="form-control mb-3 ms-3" placeholder="0" value={minReadySeconds} onChange={(e) => setMinReadySeconds(e.target.value)} />
                            <strong className="ms-3">Revision History Limit</strong>
                            <input type="number" className="form-control mb-3 ms-3" placeholder="10" value={revisionHistoryLimit} onChange={(e) => setRevisionHistoryLimit(e.target.value)} />

                            <strong>VolumeClaim Template</strong> <br/>
                            <strong className="ms-3">PVC Name</strong>
                            <input type="text" className="form-control mb-3 ms-3" placeholder="my-pvc" value={pvcName} onChange={(e) => setPVCName(e.target.value)} />
                            <strong className="ms-3">Access Modes</strong>
                            <select className="form-select mb-3 ms-3" aria-label="Default select example" onChange={(e) => setAccessModes(e.target.value)}>
                              <option value="ReadWriteOnce">ReadWriteOnce</option>
                              <option value="ReadOnlyMany">ReadOnlyMany</option>
                              <option value="ReadWriteMany">ReadWriteMany</option>
                            </select>
                            <strong className="ms-3">Storage Class Name</strong>
                            <input type="text" className="form-control mb-3 ms-3" placeholder="standard" value={storageClassName} onChange={(e) => setStorageClassName(e.target.value)} />
                            <strong className="ms-3">Storage Size</strong>
                            <input type="text" className="form-control mb-3 ms-3" placeholder="1Gi" value={storageSize} onChange={(e) => setStorageSize(e.target.value)} />
                        </div>
                      </div>
                    </div>
        ...
        <div className="result">
          ...
          <div className="cmds">
            <h5>Commands</h5>
            <div className="cmd bg-light mb-4 align-content-center " style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl apply -f stateFulSet.yaml</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl get Stateful Set</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl get describe Stateful Set {name}</div>
            <div className="cmd bg-light mb-4 align-content-center" style={{width: "550px", height: "40px", textAlign: "center", borderRadius: "8px"}}>kubectl delete -f stateFulSet.yml</div>
          </div>
        </div>
      </div>
    </>
